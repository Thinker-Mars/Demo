...
public class FunKit {
	 ...
	 * @return 是否包含
	 */
	public static boolean collectKey(Object obj, String key) {
		if (obj == null) {
			return false;
		}
		...
		if (obj instanceof JSONObject) {
			...
			if(((JSONObject) obj).get(key) == null){
				return false;
			}
			if(((JSONObject) obj).get(key).toString().trim().equals("")){
				return false;
			}
			return true;
		}
		if (obj instanceof Map) {
			...
			if(((Map<?, ?>) obj).get(key).toString().trim().equals("")){
				return false;
			}
			return true;
		}
